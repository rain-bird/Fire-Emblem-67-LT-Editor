...
#Altered to be a unit-based function, as funds are now unit specific.
def give_money(self: Event, global_unit, money: int, flags=None):
    flags = flags or set()
    
    unit = self._get_unit(global_unit)
    if not unit:
        self.logger.error("give_money: Couldn't find unit with nid %s" % global_unit)
        return
    
    party_nid = self.game.current_party
    banner_flag = 'no_banner' not in flags

    action.do(action.GainMoney(unit, money))
    #action.do(action.UpdateRecords('money', (party_nid, money)))
    if banner_flag:
        if money >= 0:
            b = banner.Advanced('{name} got <blue>{money}</> gold.'.format(name = unit.name, money = str(money)), 'Item')
        else:
            b = banner.Advanced('{name} lost <blue>{money}</> gold.'.format(name = unit.name, money = str(money * -1)), 'ItemBreak')
        self.game.alerts.append(b)
        self.game.state.change('alert')
        self.state = 'paused'
...
def choice(self: Event, nid: NID, title: str, choices: TableRows, row_width: int = 0, orientation: Orientation = Orientation.VERTICAL,
           ...
           dimensions: Optional[Tuple[str, str]] = None, text_align: HAlignment = HAlignment.LEFT, scale: int = 1, flags=None):
    flags = flags or set()
    ...
    self.game.memory['player_choice'] = (nid, header, data, row_width,
                                         ...
                                         arrows, scroll_bar, text_align, backable, event_context, scale)
